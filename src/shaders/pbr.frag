...
// Build TBN matrix
// Using local gradient of position and UV coords to derive tangent if not
// provided See https://jcgt.org/published/0009/03/04/paper.pdf Section 3.3
...
void main() {
  ...
  vec3 emissiveColor = uMaterial.emissiveColor;
#if defined(EMISSIVE_TEXTURE)
  emissiveColor *= texture(uEmissiveTexture, texCoord).rgb;
#endif

  /////////////////
  // Metalness and Roughness calc

  float perceivedRoughness = uMaterial.roughness;
  ...
  float clearCoatStrength = uClearCoat.factor;
#if defined(CLEAR_COAT_TEXTURE)
  clearCoatStrength *= texture(uClearCoatTexture, texCoord).r;
#endif
  clearCoatStrength = clamp(clearCoatStrength, 0.0, 1.0);

  float clearCoatPerceivedRoughness = uClearCoat.roughness;
...
// https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_materials_anisotropy#individual-lights
#if defined(ANISOTROPY_LAYER)
  float anisotropy = uAnisotropyLayer.factor;
  ...
  configureAnisotropyInfo(anisotropyDir, anisotropy, anisotropicT, anisotropicB,
                          view, alphaRoughness, anisoInfo);
...
// Modifies fresnel terms
  ...
  float specularWeight = uSpecularLayer.factor;
#if defined(SPECULAR_LAYER_TEXTURE)
  specularWeight *= texture(uSpecularLayerTexture, texCoord).a;
#endif
  // The F0 color of the specular reflection (linear RGB)
  vec3 specularLayerColor = uSpecularLayer.colorFactor;
...
// Direct lighting
  ...
  for (int iLight = 0; iLight < LIGHT_COUNT; ++iLight) {
    ...
    configureLightInfo(normalize(toLightVec),
                       uLightColors[iLight] * attenuation, n, view, n_dot_v, l);
    ...
    vec3 fresnel =
        fresnelSchlick(specularColor_f0, specularColor_f90, l.v_dot_h);
...
// TODO get rid of skip when implementation is ready
    ...
    vec3 ccFresnel = fresnelSchlick(clearCoatCoating_f0, clearCoatCoating_f90,
                                    cc_l.v_dot_h) *
                     clearCoatStrength;
  ...
  iblClearCoatContrib = computeIBLSpecular(clearCoatRoughness, cc_n_dot_v,
                                           clearCoatCoating_f0, cc_reflection);
...
// Scale if both direct lighting and ibl are enabled
  ...
  diffuseContrib *= uComponentScales[DirectDiffuse];
  specularContrib *= uComponentScales[DirectSpecular];
  ...
  fragmentColor.rgb =
      (fragmentColor.rgb * (OneM_ccFresnelGlbl)) + ttlClearCoatContrib;
