...
void printPathPoint(int run,
                    int step,
                    const Magnum::Vector3& p,
                    float distance) {
  ESP_VERY_VERBOSE() << run << "," << step << "," << p << "," << distance;
}
...
void NavTest::PathFinderLoadTest() {
  ...
  for (int i = 0; i < 100000; ++i) {
    ...
    if (foundPath) {
      ...
      const Magnum::Vector3& pathStart = path.points.front();
      const Magnum::Vector3& pathEnd = path.points.back();
      const Magnum::Vector3 end = pf.tryStep(pathStart, pathEnd);
...
void printRandomizedPathSet(esp::nav::PathFinder& pf) {
  ...
  for (int i = 0; i < 100; ++i) {
    const float r = 0.1;
    Magnum::Vector3 rv(random.uniform_float(-r, r), 0,
                       random.uniform_float(-r, r));
    Magnum::Vector3 rv2(random.uniform_float(-r, r), 0,
                        random.uniform_float(-r, r));
...
void NavTest::PathFinderTestCases() {
  ...
  testPath.requestedStart = Magnum::Vector3(-6.493, 0.072, -3.292);
  testPath.requestedEnd = Magnum::Vector3(-8.98, 0.072, -0.62);
  ...
  testPath.requestedEnd =
      testPath.requestedStart + Magnum::Vector3(0.01, 0.0, 0.01);
  pf.findPath(testPath);
...
void NavTest::PathFinderTestNonNavigable() {
  ...
  const Magnum::Vector3 nonNavigablePoint{1e2, 1e2, 1e2};

  const Magnum::Vector3 resultPoint = pf.tryStep(
      nonNavigablePoint, nonNavigablePoint + Magnum::Vector3{0.25, 0, 0});

  CORRADE_COMPARE_AS((nonNavigablePoint - resultPoint).length(), 1.0e-3,
...
void NavTest::PathFinderTestSeed() {
  ...
  pf.seed(1);
  Magnum::Vector3 firstPoint = pf.getRandomNavigablePoint();
  pf.seed(1);
  Magnum::Vector3 secondPoint = pf.getRandomNavigablePoint();
  CORRADE_COMPARE(firstPoint, secondPoint);

  // Different seeds should produce different points
  pf.seed(2);
  Magnum::Vector3 firstPoint2 = pf.getRandomNavigablePoint();
  pf.seed(3);
  Magnum::Vector3 secondPoint2 = pf.getRandomNavigablePoint();
  ...
  pf.seed(4);
  Magnum::Vector3 firstPoint3 = pf.getRandomNavigablePoint();
  Magnum::Vector3 secondPoint3 = pf.getRandomNavigablePoint();
