...
TEST_F(PhysicsManagerTest, DiscreteContactTest) {
  ...
  if (physicsManager_->getPhysicsSimulationLibrary() !=
    ...
    // set box 0 to VISUALIZATION_ONLY
    physicsManager_->setObjectMotionType(
        objectId0, esp::physics::MotionType::VISUALIZATION_ONLY);
    // can still be used as a shadow collision shape for discrete contact
    // testing against active objects.
    ASSERT_TRUE(physicsManager_->contactTest(objectId0));
    // will not be found by other objects during discrete contact checking
    ASSERT_FALSE(physicsManager_->contactTest(objectId1));
  }
}
