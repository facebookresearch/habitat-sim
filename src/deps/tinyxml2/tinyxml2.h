...
class TINYXML2_LIB XMLUtil {
 public:
  ...
  inline static bool StringEqual(const char* p,
                                 const char* q,
                                 int nChar = INT_MAX) {
    if (p == q) {
      return true;
...
*/
  ...
  in the XML tree will be conditionally visited and the host will be called back
  via the XMLVisitor interface.

  This is essentially a SAX interface for TinyXML-2. (Note however it doesn't
  re-parse the XML for the callbacks, so the performance of TinyXML-2 is
...
*/
  ...
  The QueryIntText() and similar functions provide a safe and easier way to get
  ...
  cannot be converted to the requested type, and XML_NO_TEXT_NODE if there is no
  child text to query.
