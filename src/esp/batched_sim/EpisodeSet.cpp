...
EpisodeSet generateBenchmarkEpisodeSet(int numEpisodes, const BpsSceneMapping& sceneMapping) {
  ...
  addFreeObject(set, "024_bowl", Mn::Range3D({-0.09557099640369415, -0.12427099794149399, 0.05447899550199509}, {0.06589200347661972, 0.03689299523830414, -0.0005300004268065095}), sceneMapping);
  addFreeObject(set, "003_cracker_box", Mn::Range3D({-0.048785001039505005, -0.09616000950336456, 0.21019400656223297}, {0.02301499992609024, 0.06787599623203278, -0.0032430035062134266}), sceneMapping);
  addFreeObject(set, "010_potted_meat_can", Mn::Range3D({-0.08382699638605118, -0.05660400539636612, 0.08035500347614288}, {0.018257999792695045, 0.0034989966079592705, -0.0031880023889243603}), sceneMapping);
  addFreeObject(set, "002_master_chef_can", Mn::Range3D({-0.06831300258636475, -0.06094900891184807, 0.13999000191688538}, {0.03421600162982941, 0.04142799228429794, -0.00018700220971368253}), sceneMapping);
  addFreeObject(set, "004_sugar_box", Mn::Range3D({-0.032214999198913574, -0.06379300355911255, 0.1760459989309311}, {0.017280999571084976, 0.030368993058800697, 3.0998555303085595e-05}), sceneMapping);
  addFreeObject(set, "005_tomato_soup_can", Mn::Range3D({-0.0431240014731884, 0.05014599487185478, 0.10193400084972382}, {0.024786999449133873, 0.11788899451494217, 7.90045305620879e-05}), sceneMapping);
  addFreeObject(set, "009_gelatin_box", Mn::Range3D({-0.06747700273990631, -0.05879899859428406, 0.02952899970114231}, {0.02192699909210205, 0.042309001088142395, -0.0005450012977235019}), sceneMapping);
  addFreeObject(set, "008_pudding_box", Mn::Range3D({-0.0684640035033226, -0.04525500163435936, 0.038391999900341034}, {0.069473996758461, 0.08350100368261337, -0.0004969995934516191}), sceneMapping);
  addFreeObject(set, "007_tuna_fish_can", Mn::Range3D({-0.06882800161838531, -0.06490200012922287, 0.030319999903440475}, {0.01673099957406521, 0.0206379983574152, -0.003218000056222081}), sceneMapping);
