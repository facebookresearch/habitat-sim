...
namespace physics {
    ...
    BulletRigidObject::BulletRigidObject(scene::SceneNode* parent) : RigidObject{parent} {};
    ...
    bool BulletRigidObject::initializeObject(
        ...
        if(bInertia[0] == 0. && bInertia[1] == 0. && bInertia[2] == 0. ){
            //Alex TODO: allow bullet to compute the inertia tensor if we don't have one
            bObjectShape_->calculateLocalInertia(metaData.mass, bInertia); //overrides bInertia
            LOG(INFO) << "Automatic object inertia computed: " << bInertia.x() << " " << bInertia.y() << " " << bInertia.z();
        }else{
            LOG(INFO) << "User provided object inertia " << bInertia.x() << " " << bInertia.y() << " " << bInertia.z();
        }
