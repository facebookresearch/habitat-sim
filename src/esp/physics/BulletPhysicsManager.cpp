...
bool BulletPhysicsManager::initPhysics(scene::SceneNode* node,
  ...
  sceneNode_ = std::dynamic_pointer_cast<physics::RigidObject, physics::BulletRigidObject>(std::make_shared<physics::BulletRigidObject>(physicsNode_));
...
bool BulletPhysicsManager::addScene(
  ...
  bool sceneSuccess = std::dynamic_pointer_cast<physics::BulletRigidObject, physics::RigidObject>(sceneNode_)->initializeScene(meshGroup, *bWorld_);
  LOG(INFO) << "Init scene done";

  return sceneSuccess;
}

int BulletPhysicsManager::addObject(
    ...
    DrawableGroup* drawables) 
{
  ...
  //int resObjectID = resourceManager->addObject(configFile, physicsNode_, drawables);
