...
PathFinder::Impl::getTopDownView(const float res, const float y_axis_val) {
  ...
  for (int h = 0; h < heightResolution; h++) {
    ...
    for (int w = 0; w < widthResolution; w++) {
      vec3f point = vec3f(curWidth, y_axis_val, curHeight);
...
template Magnum::Vector3
PathFinder::tryStep<Magnum::Vector3>(const Magnum::Vector3 &,
...
template Magnum::Vector3
PathFinder::snapPoint<Magnum::Vector3>(const Magnum::Vector3 &pt);
...
std::vector<std::vector<bool>> PathFinder::getTopDownView(const float res, const float y_axis_val) {
  return pimpl_->getTopDownView(res, y_axis_val);
}
