...
#include "esp/metadata/managers/ManagedContainerBase.h"
 ...
 */
template <class T>
class AttributesManager : public ManagedContainer<T> {
 public:
  ...
  AttributesManager(esp::assets::ResourceManager& resourceManager,
      ...
      : ManagedContainer<T>::ManagedContainer(resourceManager, attrType) {}
...
auto AttributesManager<T>::createObjectAttributesFromJson(
  ...
  AttribsPtr attributes = this->initNewObjectInternal(configFilename);
...
bool AttributesManager<T>::setJSONAssetHandleAndType(
  ...
  return false;
}  // AttributesManager<AttribsPtr>::setJSONAssetHandleAndType
