 ...
 public:
  ...
  attributes::AbstractPrimitiveAttributes::ptr createObject(
    ...
    if (primObjType == PrimObjTypes::END_PRIM_OBJ_TYPES) {
      Mn::Error{} << "AssetAttributesManager::createObject : Illegal "
  ...
  std::vector<std::string> getTemplateHandlesByPrimType(
    ...
    if (primType == PrimObjTypes::END_PRIM_OBJ_TYPES) {
      Mn::Error{} << "AssetAttributesManager::getTemplateHandlesByPrimType : "
  ...
  void setDefaultObject(
      ...
      override {
    Mn::Warning{} << "AssetAttributesManager::setDefaultObject : Overriding "
 ...
 protected:
  ...
  bool verifyTemplateHandle(const std::string& templateHandle,
    ...
    if (std::string::npos == templateHandle.find(attrType)) {
      Mn::Error{} << "AssetAttributesManager::verifyTemplateHandle : Handle : "
                  << templateHandle
  ...
  attributes::AbstractPrimitiveAttributes::ptr initNewObjectInternal(
    ...
    if (primTypeConstructorMap_.count(primClassName) == 0) {
      Mn::Error{} << "AssetAttributesManager::buildPrimAttributes : No "
                     "primitive class"
