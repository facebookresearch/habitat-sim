...
void main() {
  vec4 center_c = texture2D(texSource, textureCoordinates);
  float center_d = texture2D(texLinearDepth, textureCoordinates).x;

  vec4 c_total = center_c;
  float w_total = 1.0;

  for (float r = 1; r <= KERNEL_RADIUS; ++r) {
    vec2 uv = textureCoordinates + g_InvResolutionDirection * r;
