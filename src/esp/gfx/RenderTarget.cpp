...
struct RenderTarget::Impl {
  ...
  void blitRgbaTo(Mn::GL::AbstractFramebuffer& target, const Mn::Range2Di& targetRectangle) {
    CORRADE_ASSERT(
        flags_ & Flag::RgbaAttachment,
    ...
    CORRADE_ASSERT(
        ...
        "RenderTarget::Impl::blitRgbaTo(): target framebuffer has a size of" << targetRectangle.size() << "but expected" << framebuffer_.viewport().size(), );

    framebuffer_.mapForRead(RgbaBufferAttachment);
    Mn::GL::AbstractFramebuffer::blit(
        framebuffer_, target, framebuffer_.viewport(),
        targetRectangle, Mn::GL::FramebufferBlit::Color,
...
void RenderTarget::blitRgbaTo(Mn::GL::AbstractFramebuffer& target, const Mn::Range2Di& targetRectangle) {
  pimpl_->blitRgbaTo(target, targetRectangle);
}

void RenderTarget::blitRgbaToDefault() {
  pimpl_->blitRgbaTo(Mn::GL::defaultFramebuffer, Mn::GL::defaultFramebuffer.viewport());
}
