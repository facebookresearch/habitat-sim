...
struct PlayerCallbacks
{
    ...
    using DeleteAssetInstance =
        std::function<void(GfxReplayNode*)>;

    using SetAssetInstanceTransform =
        std::function<void(GfxReplayNode*, Magnum::Vector3, Magnum::Quaternion)>;

    using SetAssetInstanceSemanticId =
        std::function<void(GfxReplayNode*, int)>;
...
/**
 * @brief Factory method to create a PlayerCallbacks that handles esp::scene::SceneNode
 */
static PlayerCallbacks createSceneGraphPlayerCallbacks()
{
    ...
    callbacks.setAssetInstanceTransform_ = [](gfx::replay::GfxReplayNode* node, Magnum::Vector3 translation, Magnum::Quaternion rotation) {
      auto sceneNode = reinterpret_cast<scene::SceneNode*>(node);
      sceneNode->setTranslation(translation);
      sceneNode->setRotation(rotation);
    };
    callbacks.setAssetInstanceSemanticId_ = [](gfx::replay::GfxReplayNode* node, int semanticId) {
      auto sceneNode = reinterpret_cast<scene::SceneNode*>(node);
