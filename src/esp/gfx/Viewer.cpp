...
constexpr float visualSensorHeight = 1.5f;
...
Viewer::Viewer(const Arguments& arguments)
  ...
  visualSensorNode_ = &agentBodyNode_->createChild();

  visualSensorNode_->translate({0.0f, visualSensorHeight, 0.0f});
  ...
  renderCamera_->node().setTransformation(
      visualSensorNode_->absoluteTransformation());
...
void Viewer::keyPressEvent(KeyEvent& event) {
  ...
  switch (key) {
    ...
    case KeyEvent::Key::Up:
      controls_(*visualSensorNode_, "lookUp", lookSensitivity, false);
      break;
    case KeyEvent::Key::Down:
      controls_(*visualSensorNode_, "lookDown", lookSensitivity, false);
      break;
