...
void RendererStandalone::colorImageInto(const Magnum::Range2Di& rectangle, const Mn::MutableImageView2D& image) {
  ...
  CORRADE_ASSERT(rectangle.max() <= tileCount()*tileSize(),
    "RendererStandalone::colorImageInto():" << rectangle << "doesn't fit in a size of" << tileCount()*tileSize(), );
  CORRADE_ASSERT(image.size() == rectangle.size(),
    "RendererStandalone::colorImageInto(): expected image size of" << rectangle.size() << "pixels but got" << image.size(), );
...
void RendererStandalone::depthImageInto(const Magnum::Range2Di& rectangle, const Mn::MutableImageView2D& image) {
  /* Deliberately not checking that image.format() == depthFramebufferFormat()
     ...
     float) */
  CORRADE_ASSERT(rectangle.max() <= tileCount()*tileSize(),
    "RendererStandalone::depthImageInto():" << rectangle << "doesn't fit in a size of" << tileCount()*tileSize(), );
  CORRADE_ASSERT(image.size() == rectangle.size(),
    "RendererStandalone::depthImageInto(): expected image size of" << rectangle.size() << "pixels but got" << image.size(), );
  return state_->framebuffer.read(rectangle, image);
}
