...
// a uint64_t, M, the number of vertices within the chunk;

// M uint32_t, vertex indices in the *original* mesh (in other words, a lookup
// table from new index to original index);

// a uint64_t, K, the number of entries in the elemnt index array, should be 4x
// of the number of faces

// K uint32_t, the element index array (new index, NOT the original index);
// ===================================================

//  this binary can be generated by modifying the splitMesh() in file
//  PTexLib.cpp in ReplicaSDK.
//  and the name is hard-coded as "sub_meshes.bin" in the simulator.
...
std::vector<PTexMeshData::MeshData> loadSubMeshes(
  ...
  for (uint64_t iSubMesh = 0; iSubMesh < numSubMeshes; ++iSubMesh) {
    uint64_t numVertices = 0;
    file.read((char*)&numVertices, sizeof(uint64_t));

    // load the lookup table which maps the new index of current sub-mesh to the
    // old index of the original mesh
    std::vector<uint32_t> refdVerts(numVertices);
    file.read((char*)refdVerts.data(), sizeof(uint32_t) * numVertices);

    // fill in the vbo and nbo
    subMeshes[iSubMesh].vbo.resize(numVertices);
    subMeshes[iSubMesh].nbo.resize(numVertices);
    for (size_t jVertex = 0; jVertex < numVertices; ++jVertex) {
      uint32_t originalIndex = refdVerts[jVertex];
      subMeshes[iSubMesh].vbo[jVertex] = mesh.vbo[originalIndex];
      subMeshes[iSubMesh].nbo[jVertex] = mesh.nbo[originalIndex];
    }

    // load the ibo
    uint64_t numEntries = 0;
    file.read((char*)&numEntries, sizeof(uint64_t));
    subMeshes[iSubMesh].ibo.resize(numEntries);
    file.read((char*)subMeshes[iSubMesh].ibo.data(), sizeof(uint32_t) * numEntries);
...
void PTexMeshData::loadMeshData(const std::string& meshFile) {
  ...
  if (splitSize_ > 0.0f) {
    ...
    std::string subMeshesFilename =
        Corrade::Utility::Directory::join(atlasFolder_, "../sub_meshes.bin");
