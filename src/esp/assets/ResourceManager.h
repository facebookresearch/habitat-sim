...
/**
 * @brief Loaded asset and resource manager. Singleton class responsible for
...
class ResourceManager {
 public:
   ...
   * a child of the specified @ref scene::SceneNode. If these are not specified,
   * the assets are loaded, but no new @ref gfx::Drawable is added for the scene
   * (i.e. it will not be rendered).
   ...
   * it was loaded. See @ref translateMesh.
  ...
  /**
   * @brief Retrieve the meta data for a particular asset. This includes
   * identifiers for meshes, textures, materials, and a component heirarchy.
 ...
 protected:
  ...
  /**
   * @brief Load meshes from importer into assets, compute bounding boxes,
   * upload mesh data to GPU, and update metaData for an asset to link meshes to
   * that asset.
   * @param importer The importer already loaded with information for the asset.
   * @param metaData The asset's @ref MeshMetaData object.
   */
   ...
   * component transformation heirarchy for loaded assets. Maps absolute path
   * keys to metadata.
   */
   ...
   * templates. Templates are used by @ref physics::PhysicsManager to instance
  ...
  /**
   * @brief List of object library keys. See @ref physicsObjectLibrary_, @ref
   * collisionMeshGroups_. NOTE: can't get keys from the map (easily), so store
   * them for iteration.
  ...
  enum ShaderType {
    /**
     * @brief Shader program for instance mesh data. See @ref
     * gfx::PrimitiveIDShader, @ref GenericInstanceMeshData, @ref
     * Mp3dInstanceMeshData, @ref AssetType::INSTANCE_MESH, @ref
     * loadInstanceMeshData.
     */
    INSTANCE_MESH_SHADER = 0,

    /**
     * @brief Shader program for PTex mesh data. See @ref gfx::PTexMeshShader,
     * @ref gfx::PTexMeshDrawable, @ref loadPTexMeshData, @ref PTexMeshData.
     */
    PTEX_MESH_SHADER = 1,

    /**
     * @brief Shader program for flat shading with uniform color. Used to render
     * object identifier or semantic types (e.g. chair, table, etc...). Also the
    ...
    /**
     * @brief Shader program for vertex color shading. Used to render meshes
     * with per-vertex colors defined.
     */
    VERTEX_COLORED_SHADER = 3,

    /**
     * @brief Shader program for meshes with textured defined.
     */
    TEXTURED_SHADER = 4,
  };
   ...
   * Magnum::GL::AbstractShaderProgram. See @ref getShaderProgram.
   */
  std::map<ShaderType, std::shared_ptr<Magnum::GL::AbstractShaderProgram>>
      shaderPrograms_;

  /**
   * @brief Returns a pointer to the specified shader program. Creates a new
   * shader program for @ref ShaderType if it does not exist.
