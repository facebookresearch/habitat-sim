...
int ResourceManager::loadObject(PhysicsObjectAttributes& objectTemplate,
  ...
  // assign semantic ID to rendering mesh if specified
  if (objectTemplate.existsAs(DataType::INT, "semanticId")) {
    int semanticId = objectTemplate.getInt("semanticId");
    if (semanticId != ID_UNDEFINED) {
      // set the objectID for use when creating the drawables
      setAllObjectIDs(semanticId, resourceDict_.at(renderMeshFilename).root);
    }
  }
...
void ResourceManager::createDrawable(
  ...
  if (shaderType == PTEX_MESH_SHADER) {
    ...
    node.addFeature<gfx::GenericDrawable>(*shader, mesh, group, texture, color);
  }
}
