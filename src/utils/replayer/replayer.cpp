...
Replayer::Replayer(const Arguments& arguments): Mn::Platform::Application{arguments, Mn::NoCreate} {
  Cr::Utility::Arguments args;
  args.addArrayArgument("json").setHelp("json", "gfx-replay JSON file(s) to use")
    .addArrayOption('P', "preload").setHelp("preload", "composite file(s) to preload", "file.glb")
    .addOption("size", "512 384").setHelp("size", "environment size", "\"X Y\"")
    .addBooleanOption("classic").setHelp("classic", "use the classic renderer")
    .addBooleanOption("profile").setHelp("profile", "print frame profiling info to the console")
  ...
  create(Configuration{}
    .setSize(args.value<Mn::Vector2i>("size")*esp::sim::AbstractReplayRenderer::environmentGridSize(fileCount))
    .setTitle("Replayer"));

  arrayReserve(jsonFiles_, fileCount);
  fileKeyframeOffsets_ = Cr::Containers::Array<std::size_t>{Cr::ValueInit, fileCount + 1};
  for(std::size_t i = 0; i != fileCount; ++i) {
    const auto filename = args.arrayValue<Cr::Containers::StringView>("json", i);

    /* Just tokenize the file and process the bare minimum needed to be able to
       go through the keyframe list */
    Cr::Containers::Optional<Cr::Utility::Json> jsonFile = Cr::Utility::Json::fromFile(filename);
    ...
    if(!(jsonKeyframes = (*root).find("keyframes")) || !jsonFile->parseArray(*jsonKeyframes))
    ...
    maxFrameCount_ = Mn::Math::max(maxFrameCount_, fileKeyframeOffsets_[i + 1] - fileKeyframeOffsets_[i]);
  }

  Mn::Debug{} << "Playing" << maxFrameCount_ << "frames from" << fileCount << "files";
  ...
  std::vector<esp::sensor::SensorSpec::ptr> sensorSpecifications;
  {
    ...
    /* Account for DPI scaling, i.e. take the actual framebuffer resolution
       ...
       in the arguments */
    pinholeCameraSpec->resolution = Mn::EigenIntegration::cast<esp::vec2i>((framebufferSize()/esp::sim::AbstractReplayRenderer::environmentGridSize(fileCount)).flipped());
...
void Replayer::drawEvent() {
  ...
  for (std::size_t envIndex = 0; envIndex < jsonFiles_.size(); ++envIndex) {
    const Cr::Containers::ArrayView<const Cr::Containers::StringView> keyframesForEnvironment = keyframes_.slice(fileKeyframeOffsets_[envIndex], fileKeyframeOffsets_[envIndex + 1]);
    ...
    if (frameIndex_ < keyframesForEnvironment.size()) {
      replayRenderer_->setEnvironmentKeyframe(envIndex,
        ...
        Cr::Utility::formatString("{{\"keyframe\":{}}}", keyframesForEnvironment[frameIndex_]));
    }
