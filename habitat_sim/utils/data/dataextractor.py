...
class DataExtractor(object):
    ...
    def __init__(self, scene_filepath, label):
        ...
        self.pose_extractor = PoseExtractor(self.topdown_view, self.sim.pathfinder, self.res)
        self.poses = self.pose_extractor.extract_poses(dist=20, label=self.label) # list of poses
...
class PoseExtractor(object):
    ...
    def _get_pathfinder_reference_point(self):
        ...
        startz = self.pathfinder.get_random_navigable_point()[1] # Can't think of a better way to get a valid z-axis value
    ...
    def _compute_quat(self, position, cam_normal):
        ...
        theta = -180 * np.arccos(np.dot(position, cam_normal) / (norm(position) * norm(cam_normal))) / np.pi
    ...
    def _bfs(self, point, label):
        def get_neighbors(p):
            r, c = p
            step = 2
            return [(r - step, c - step), (r - step, c), (r - step, c + step), (r, c - step),
                (r, c + step), (r + step, c - step), (r + step, c), (r + step, c + step)]

        is_valid = lambda row, col: 0 <= row < len(self.topdown_view) and 0 <= col < len(self.topdown_view[0])
        visited = set() # Can use the topdown view as visited set to save space at cost of time to reset it for each bfs
        q = collections.deque([point])
        while q:
            cur = q.popleft()
            visited.add(cur)
            if self.is_point_of_interest(cur, label): # Looking for False labels right now. Change this param later
                return cur
