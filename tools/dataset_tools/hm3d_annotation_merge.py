...
# and contains at least 2 files that vary by extension :
#   <YYYYYYYYYYY>.semantic.<glb|txt>, where
#     <XXXXX> represents a numeric value from 00000 -> 00999
#     <YYYYYYYYYYY> represents an alpha-numeric hash representing the scene
#
# This is the format given in the zip files from Appen

HM3D_ANNOTATION_SRC_DIR = "/home/john/Datasets In Progress/HM3D_Semantic/Appen_60"
...
def buildFileListing():
    ...
    for src_dir, dirname_full in dir_listing:
        ...
        dest_dir_list = ut.get_directories_matching_regex(
            HM3D_DEST_DIR, re.compile(dirname + "$")
        )
        # restrict allowable results to only 1
        if len(dest_dir_list) == 1:
            scene_dest_dir = join(dest_dir_list[0][0], dest_dir_list[0][1])
            src_files = {}
            for src_full_dir, _, src_filename in src_file_list:
                ...
                src_files["dest_" + key + "_path"] = join(scene_dest_dir, src_filename)
            ...
            file_names_and_paths[src_dirname_full] = {"dest_dir": scene_dest_dir}
            for k, v in src_files.items():
                file_names_and_paths[src_dirname_full][k] = v

        else:
            print(
                "Problem with source dir {} : Unable to find destination dir due to {} matching \
                 destinations in current HM3D dataset so skipping this source dir.".format(
...
def main():
    ...
    failures = {}
    for src_dir, data_dict in file_names_and_paths.items():
