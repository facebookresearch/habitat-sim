...
class Simulator(SimulatorBackend):
    ...
    def _init_sensor(self, sensor_spec: SensorSpec, agent_id: int = None) -> None:
        ...
        if agent_id is not None:
            ...
            # populate to-be-removed list of per-agent dicts with sensor
            self.__sensors[agent_id][sensor_spec.uuid] = sensor

            if sensor_spec.sensor_type != SensorType.AUDIO:
                # create buffer and put in member variables
                self.__create_obs_buffer(
                    sensor_spec,
                    sensor,
                    self.__obs_buffers[agent_id],
            ...
            if sensor_spec.sensor_type != SensorType.AUDIO:
                # create buffer and put in member variables
                self.__create_obs_buffer(
                    sensor_spec, sensor, self.__obs_buffers_, self.__image_views_
                )

    def __create_obs_buffer(
        self,
        sensor_spec: SensorSpec,
        sensor: Sensor,
        obs_buffer_dict: Dict[str, SensorObservation],
        image_view_dict: Dict[str, mn.MutableImageView2D],
    ) -> None:
        r"""Creates an observation buffer, and possibly an image view, of correct type and
        dimensions for the given sensor as described by the sensor_spec, and places the
        appropriate constructs in t
        :param sensor_spec: The SensorSpec describing the sensor
        :param sensor: The Sensor object responsible for the observation buffer
        :param obs_buffer_dict: dictionary holding all the observation buffers,
            keyed by the sensor's uuid
        :param image_view_dict: dictionary holding all the image view corresponding to
            observation buffer (only built for non-torch obs buffers)
        """
        if self.renderer is not None:
        ...
        if sensor_spec.gpu2gpu_transfer:
            ...
            if sensor_spec.sensor_type == SensorType.SEMANTIC:
                obs_buffer_dict[sensor_spec.uuid] = torch.empty(
            ...
            if sensor_spec.sensor_type == SensorType.SEMANTIC:
                ...
                image_view_dict[sensor_spec.uuid] = mn.MutableImageView2D(
            ...
            obs_buffer_dict[sensor_spec.uuid] = obs_buffer
