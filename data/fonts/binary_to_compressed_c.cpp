...
// used 32-bits constants it would require take 11 bytes of source code to
// encode 4 bytes, and be endianness dependent) Note that even with compression,
// the output array is likely to be bigger than the binary file.. Load
// compressed TTF fonts with
...
static bool binary_to_compressed_c(const char* filename,
                                   const char* symbol,
...
int main(int argc, char** argv) {
  ...
  bool ret = binary_to_compressed_c(argv[argn], argv[argn + 1],
                                    use_base85_encoding, use_compression);
  if (!ret)
