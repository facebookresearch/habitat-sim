...
if __name__ == "__main__":
    ...
    with habitat_sim.Simulator(cfg) as sim:
        ...
        # Show the scene for 2 seconds with no voxelization
        simulate(sim, dt=2, get_frames=True, data=data)

        # Voxelize the scene, and set the scene voxelizaton to be drawn
        resolution = 2000000
        sim.voxelize_scene(resolution)
        sim.set_scene_voxelization_draw(True, "Boundary")

        # Show for 2 seconds
        simulate(sim, dt=2, get_frames=True, data=data)

        voxelization = sim.get_scene_voxelization()
