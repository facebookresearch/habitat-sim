...
import habitat_sim
import habitat_sim.robots.spot_robot as spot_robot
from examples.settings import default_sim_settings, make_cfg
from habitat_sim import physics
from habitat_sim.logging import LoggingContext, logger


def create_platform_scene(sim: habitat_sim.Simulator):
    """
    Generate a primitive platform to act as the stage, including NavMesh, for an empty scene.
    """
...
# ---------------------------------------------------------
# Test functions for Spot wrapper
# ---------------------------------------------------------
def test_load_spot_interface(application):
    ...
    assert os.path.exists(
        robot_path
    ), "Download the Spot asset and extract to `data/robots/`."

    spot = spot_robot.SpotRobot(robot_path, sim, fixed_base=False)
    spot.reconfigure()
    assert spot.get_robot_sim_id() == 1  # 0 is the ground plane
    print(spot.get_link_and_joint_names())
    if sim.pathfinder.is_loaded:
        print("Snapped robot to NavMesh")
        spot.base_pos = sim.pathfinder.snap_point(spot.base_pos)
        print(spot.base_pos)
    else:
        print("No NavMesh loaded, leaving robot at origin.")
    application.robot = spot


def increment_robot_joint(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    arm_state = application.robot.arm_joint_pos
    arm_state[application.target_joint] += 0.1
    application.robot.arm_joint_pos = arm_state
    print(f"arm_state = {arm_state}")


def decrement_robot_joint(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    arm_state = application.robot.arm_joint_pos
    arm_state[application.target_joint] -= 0.1
    application.robot.arm_joint_pos = arm_state
...
def increment_target_joint(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    application.target_joint = (application.target_joint + 1) % len(
        application.robot.arm_joint_pos
    )
    print(f"target joint = {application.target_joint}")


def decrement_target_joint(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    application.target_joint = (application.target_joint - 1) % len(
        application.robot.arm_joint_pos
    )
...
def retract_arm(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    application.robot.arm_joint_pos = application.robot.params.arm_init_params


def open_close_gripper(application):
    assert application.robot, "Press '0' to initialize the Spot first."
    print(f"gripper open = {application.robot.is_gripper_open}")
    print(f"gripper closed = {application.robot.is_gripper_closed}")
    print(f"gripper state = {application.robot.gripper_joint_pos}")
    if not application.robot.is_gripper_open:
        application.robot.open_gripper()
    else:
        application.robot.close_gripper()


# NOTE: setup keypress for test functions here.
test_functions = {
    Application.KeyEvent.Key.ZERO: test_load_spot_interface,
    Application.KeyEvent.Key.ONE: increment_robot_joint,
    Application.KeyEvent.Key.TWO: decrement_robot_joint,
    Application.KeyEvent.Key.THREE: increment_target_joint,
    Application.KeyEvent.Key.FOUR: decrement_target_joint,
    Application.KeyEvent.Key.FIVE: extend_arm,
    Application.KeyEvent.Key.SIX: retract_arm,
    Application.KeyEvent.Key.SEVEN: open_close_gripper,
}
# ---------------------------------------------------------
...
class HabitatSimInteractiveViewer(Application):
    def __init__(self, sim_settings: Dict[str, Any]) -> None:
        ...
        configuration.title = "Habitat Sim Interactive Viewer - Spot Robot Wrapper"
    ...
    def reconfigure_sim(self) -> None:
        ...
        # In "NONE" scene, create a platform
        if self.sim.config.sim_cfg.scene_id == "NONE":
            create_platform_scene(self.sim)
...
-------------
    ...
    `0`: Initialize the Spot robot wrapper.
    '1-9': Run the test functions on the initialized Spot robot.
