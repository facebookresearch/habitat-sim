...
class PLERP:
    """
    Handles LERP between a set of points.
    """
    ...
    def __init__(self, points=None) -> None:
        ...
        self.seg_times = (
            []
        )  # assigns each line segment a max time between 0-1 based on total path length
    ...
    def _recompute_times(self):
        self.seg_times = []
        self.total_path_length = 0
        for pix in range(len(self.points)):
            if pix > 0:
                self.total_path_length += (
                    self.points[pix] - self.points[pix - 1]
                ).length()
                self.seg_times.append(self.total_path_length)
        if self.total_path_length > 0:
            self.seg_times = [
                seg_time / self.total_path_length for seg_time in self.seg_times
            ]

        print(f"self.total_path_length = {self.total_path_length}")
        print(f"self.seg_times = {self.seg_times}")

    def add_point(self, p):
        self.points.append(p)
        self._recompute_times()

    def p_at(self, t: float):
        """
        Get a point at time = t [0,1].
        """
        if len(self.points) == 0:
            return mn.Vector3()
        if len(self.points) != (len(self.seg_times) + 1):
            self._recompute_times()
        if t >= 1.0:
        ...
        t_seg_ix = 0
        seg_len = self.seg_times[0]
        for seg_ix, seg_t in enumerate(self.seg_times):
            if t < seg_t:
                t_seg_ix = seg_ix
                break
        if t_seg_ix > 0:
            seg_len = self.seg_times[t_seg_ix] - self.seg_times[t_seg_ix - 1]
        seg_t = (self.seg_times[t_seg_ix] - t) / seg_len
        return mn.math.lerp(self.points[t_seg_ix + 1], self.points[t_seg_ix], seg_t)

    def reset(self, new_t=None):
        self.points = []
        self.seg_times = []
        self.total_path_length = 0
...
class HabitatSimInteractiveViewer(Application):
    ...
    def __init__(self, sim_settings: Dict[str, Any]) -> None:
        ...
        self.plerp = PLERP(
            points=[
                mn.Vector3(-1.85063, 1.32593, -0.654852),
                mn.Vector3(-2.11517, 1.31969, -0.652868),
                mn.Vector3(-2.34397, 1.30987, -0.651816),
                mn.Vector3(-2.60602, 1.31827, -0.653098),
                mn.Vector3(-2.66591, 0.905828, -0.641116),
                mn.Vector3(-2.75709, 1.63424, -0.660276),
                mn.Vector3(-2.75291, 1.40622, -0.666565),
                mn.Vector3(-2.75332, 0.753889, -0.660136),
                mn.Vector3(-2.2578, 0.736942, -0.66063),
                mn.Vector3(-1.75615, 0.753761, -0.661616),
                mn.Vector3(-1.74196, 1.4827, -0.662176),
                mn.Vector3(-2.24373, 1.48716, -0.660259),
            ]
        )
        self.reach_speed = 1.0  # linear velocity of target point
        ...
        self.humanoid_reach_point = mn.Vector3()

        self.humanoid_seq_motions_staged = {
            "test": (
                mn.Vector3(-2.08556, 0.161142, -1.27792),
                0,
                mn.Vector3(
                    [-4.151944637298584, 0.1611417979001999, -1.7563586235046387]
                ),
                mn.Quaternion(
                    mn.Vector3([0.0, 0.85263991355896, 0.0]), -0.5224990248680115
                ),
                mn.Vector3([0.0, 1.5, 0.0]),
                mn.Quaternion(
                    mn.Vector3([-0.13052533566951752, 0.0, 0.0]), 0.991445004940033
                ),
            )
        }
    ...
    def init_humanoid(
        ...
        seq_state = self.humanoid_seq_motions_staged["test"]
    ...
    def update_humanoid(self):
        ...
        if type(self.humanoid_controller) == HumanoidRearrangeController:
            # self.humanoid_reach_point = self.default_agent.scene_node.translation
            self.humanoid_controller.calculate_reach_pose(
                obj_pos=self.humanoid_reach_point
            )
    ...
    def debug_draw(self):
        ...
        self.sim.get_debug_line_render().draw_circle(
            translation=self.humanoid_reach_point,
            radius=0.02,
            color=mn.Color4.red(),
            normal=(
                self.humanoid_reach_point - self.humanoid.sim_obj.translation
            ).normalized(),
        )
    ...
    def draw_event(
        ...
        seq_t = (
            (self.sim.get_world_time() - self.plerp.time_start)
            * self.reach_speed
            * (1.0 / self.plerp.total_path_length)
        )
        ...
        if self.time_since_last_simulation >= 1.0 / self.fps:
            if self.simulating or self.simulate_single_step:
                ...
                if simulation_call is not None:
                    simulation_call()
                if self.plerp.total_path_length > 0:
                    self.humanoid_reach_point = self.plerp.p_at(seq_t)
    ...
    def mouse_move_event(self, event: Application.MouseMoveEvent) -> None:
        ...
        #
        # for hit in self.mouse_cast_results.hits:
        #    if hit.object_id not in ao_obj_ids:
        #        self.humanoid_reach_point = hit.point
        #        break
    ...
    def mouse_press_event(self, event: Application.MouseEvent) -> None:
        ...
        if self.mouse_interaction == MouseMode.GRAB and physics_enabled:
            ...
            if raycast_results.has_hits():
                ...
                self.plerp.add_point(hit_info.point)
                print(f"all points = {self.plerp.points}")
