...
salt_and_pepper_model = make_sensor_noise_model("SaltAndPepperNoiseModel", dict(gpu_device_id=1))
...
print(1 / (np.min(timeit.repeat(lambda: salt_and_pepper_model(img), repeat=10, number=1000)) / 1000))
print('\n')
...
print("Average executions of noise model per second:")
print(1 / (np.min(timeit.repeat(lambda: gaussian_model(img), repeat=10, number=1000)) / 1000))
print('\n')
...
print("Average executions of noise model per second:")
print(1 / (np.min(timeit.repeat(lambda: speckle_model(img), repeat=10, number=1000)) / 1000))
print('\n')
...
print("Average executions of noise model per second:")
print(1 / (np.min(timeit.repeat(lambda: poisson_model(img), repeat=10, number=1000)) / 1000))
